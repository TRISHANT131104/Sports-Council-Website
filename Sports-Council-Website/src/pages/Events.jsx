      ...
      <div className='w-full h-full flex justify-start flex-col items-center'>
        ...
        <div className='p-5 w-full px-20 flex flex-col justify-center items-center'>
                {events && events.length > 0 ? (
                    events.map((event) => (
                        <Link to={`/events/${event.id}`} key={event.id} className="mb-5 border p-4 shadow-lg flex flex-col justify-center items-center h-full w-full border-black">
                            <h2 className="text-3xl font-semibold">{event.Title}</h2>
                            <img src={`${event.Img}`} alt={event.Title} className="w-full max-w-[1000px] h-auto m-10 border border-black"/>
                        </Link>
                    ))
                ) : (
                    <div>Loading...</div>
